...
from kvae.utils import nll_gaussian_var_fixed
...
class KalmanVAE(nn.Module):
    def __init__(self, input_dim, hidden_dim, obs_dim, latent_dim, num_modes, beta=1):
        ...
        self.parameter_net = nn.LSTM(self.obs_dim, 32, 2, batch_first=True)
        ...
        self.mu_1 = (torch.zeros(self.latent_dim)).float()
        self.Sigma_1 = (20*torch.eye(self.latent_dim)).float()
        # Matrix modes
        ...
        # WHY ? 
        self.Q = 0.08*torch.eye(self.latent_dim).to(float) 
        self.R = 0.03*torch.eye(self.obs_dim).to(float) 
    ...
    def _interpolate_matrices(self, obs):
        ...
        inter_weight = dyn_emb.softmax(-1)
        A_t = torch.matmul(inter_weight, self.A.reshape(self.num_modes,-1)).reshape(B,T,self.latent_dim,self.latent_dim).to(float)
        C_t = torch.matmul(inter_weight, self.C.reshape(self.num_modes,-1)).reshape(B,T,self.obs_dim,self.latent_dim).to(float)
    ...
    def _filter_posterior(self, obs, A, C):
        ...
        obs = obs.unsqueeze(-1)
        mu_t = self.mu_1.expand(B,-1).unsqueeze(-1).to(float)
        Sigma_t = self.Sigma_1.expand(B,-1,-1).to(float)
        ...
        for t in range(T):
            ...
            Kalman_gain = Kalman_gain.to(float)    
            # filter: t | t
            mu_z = mu_t + torch.matmul(Kalman_gain, r.to(float))
            
            # I_ = torch.eye(self.latent_dim).to(obs.device) - torch.matmul(Kalman_gain.double(), C[:,t,:,:].double())
    ...
    def _smooth_posterior(self, A, filtered, prediction):
        ...
        mu_filt, Sigma_filt = mu_filt.to(float), Sigma_filt.to(float)

        mu_pred, Sigma_pred = prediction
        mu_pred, Sigma_pred = mu_pred.to(float), Sigma_pred.to(float)
    ...
    def _compute_elbo(self, x, mask_frames, mask_visual, x_hat, a_mu, a_log_var, a_sample, smoothed, A, C):
        ...
        decoder_x = Bernoulli(x_hat, validate_args=False)
