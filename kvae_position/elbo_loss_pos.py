...
class ELBO():
    def __init__(self, a_sample, smoothed, A_t, C_t):
        ...
        # Fix variance of a
        (B, T, *_) = self.a_sample.size()
        a_var = 0.05 * torch.eye(self.a_dim).expand(B,T,-1,-1).double().to(self.device)
     
    def compute_loss(self): 
        """ Computes loss.
        
        Returns: 
            loss:  latent_ll - elbo_kf
            latent_ll: log q(a)
        ...
        ### LGSSM 
        (B, T, *_) = self.a_sample.size()
        self.sigma_z_smoothed = torch.clamp(self.sigma_z_smoothed, min = 1e-6) # force values to be above 1e-8
        torch.save(self.sigma_z_smoothed, 'sigma_z.pt')
    ...
    def compute_a_marginal_loglikelihood(self):
        ...
        """
        q_a = MultivariateNormal(self.a_mu, torch.diag_embed(self.a_var)) # BS X T X a_dim
